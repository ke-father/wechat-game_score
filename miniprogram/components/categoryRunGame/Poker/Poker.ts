...
Page<IRunGameData, IRunGameCustom>({
    ...
    memberActionByCall () {
        ...
        // 当阶段为翻前  由大盲开拍
        if (member.currentPosition === POKER_POSITION_TYPE.SB && this.data.playStatus === POKER_ACTION_TYPE.PRE_FLOP) {
            let status = true
    ...
    Common_MemberAction () {
        ...
        if (raiseMember && (raiseMember as any) === member) this.data.isOpen = true

        // 是否全员过牌
        let checkStatus = this.data.memberList.every(i => [POKER_BEHAVIOR_TYPE.CHECK, POKER_BEHAVIOR_TYPE.FOLD].includes(i.currentAction.action))
        if (checkStatus) this.data.isOpen = true
    ...
    pre_Flop_MemberAction() {
        console.log('Pre_Flop_MemberAction')
        let member = this.data.memberList[this.data.currentMemberIndex]
        let score = 0
